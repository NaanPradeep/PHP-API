...
if(
    ...
    $user_meta->create_default_meta('language', 'en');
    $user_meta->create_default_meta('time_zone', 'America/New_York');
    $user_meta->create_default_meta('currency', 'USD');   
    $user_meta->create_default_meta('userimage', 'blank.png'); 
    $user_meta->create_default_meta('Notification_settings', $default_notification_settings);
    $user_meta->create_default_meta('minimum_bid_threshold', '0.00'); 
    ...
    $token = array( 
     ...
     $_notification_settings = array(
        ...
        "minBidThreshold" => "0.00"
     );


    $subject = _gettext("Account created successfully");

    $body = "<p><strong>Account created succesfully</strong>. 
            Verification link stays valid for <em>24 hours</em>. 
            Use the follwoing link to <em>verify</em> and <em>login</em> :&nbsp;
            <a href='http://localhost:3011/metronic8/react/demo1/verify-user?token={$otp}'>Click Here</a></p>";
    ...
    try {
        ...
        echo json_encode(array(
            "message" => _gettext("Account created successfully and verification email has been sent to your email"),
            "token" => $jwt,
            "user" => $_user,
        ...
        echo json_encode(array(
            "message" => _gettext("Account created successfully but we are unable to send verification email."),
            "token" => $jwt,
            "user" => $_user,
            "account_preference" => $_account_preference,
            "is_verified" => false,
...
// message if unable to create user or if email already exists
    ...
    if($user->emailExists()) {
        ...
        echo json_encode(array(
            "message" => _gettext("Email already exists and linked with another account"),
            "created" => false
        ));
    } else {
        ...
        echo json_encode(array(
            "message" => _gettext("Registration Failed"),
            "created" => false
        ));
    }
}
