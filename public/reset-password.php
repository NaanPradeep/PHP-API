	...
	if($current_time - $time_stamp < 180){
		...
		if($update_password){
			...
			echo json_encode(array(
				"success" => true,
				"message" => _gettext("Your password has been successfully changed")
			));
		} else {
			...
			echo json_encode(array(
				"success" => false,
				"message" => _gettext("Your password reset request failed"),
			));
		}
	} else {
		...
		echo json_encode(array(
			"success" => false,
			"message" => _gettext("Your password reset link was expired"),
		));
	}
} else {
